...
#+begin_src R :results output graphics :file img/peg_2014-122-st1-v3.png :exports both :width 1200 :height 400 :session
st0.df  %>%
    mutate(Color.Code = H) %>%    
...
**** TODO Strategy #2: 3600 bins (360 H \times 10 V)
...
# ... with 3,590 more rows
#+end_example
***** Confirme we have tidy data of H and V
...
#+begin_src R :results output graphics :file img/peg_2014-122-v3.png :exports both :width 600 :height 400 :session
library(ggplot2);
df %>% 
    select(-V, -V.count, -Color.Code) %>%
    unique %>%
    filter(Day < 100) %>%
    filter(Seq == 1) %>%
    ggplot(aes(x = Day, y = H.count, color=as.factor(Color.Code))) +
    scale_fill_manual(values=palette.list) +
    geom_bar(stat='identity', width=1) +
    ylim(0,NA) +
...
#+RESULTS:
[[file:img/peg_2014-122-v3.png]]
**** TODO Strategy #3: (360 H, fixed S=1, but V is the mode)
