...
#+RESULTS:
[[file:img/PEG_2014_small-v2.png]]
...
**** Calculate the double histogram

The same histogram is used for both strategies.
...
So, V data is spreaded, we should gather it in two columns (V,
V.count) below. After doing so, we map the range of the V value (0
to 9) to something acceptable by the =colorspace= package, so (0 to
1). The same is valid for the S, which we define as set to 1, fixed.
...
df <- df.histograms %>%
    ...
    # I don't understand why next line is necessary
    #  mutate(V = V*360 + H) %>%
    # Cosmetics
    separate(Name, sep="/", into=c("Dir", "Year", "Filename")) %>%
    select(-Dir, -Year) %>%
